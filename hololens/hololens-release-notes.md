...
ms.locfileid: "127862480"
---
# <a name="hololens-2-release-notes"></a>HoloLens wersji 2

Aby zapewnić wydajną pracę z urządzeniami HoloLens, kontynuujemy wydanie funkcji, usterek i aktualizacji zabezpieczeń. Na tej stronie możesz zobaczyć, co nowego w każdej HoloLens miesiącu. Aby uzyskać najnowszą aktualizację HoloLens 2, [](hololens-update-hololens.md#check-for-updates-and-manually-update) możesz sprawdzić aktualizacje i ręcznie zaktualizować lub pobrać pełną aktualizację flash (FFU), aby flashować urządzenie za pomocą narzędzia [Advanced Recovery Companion.](hololens-recovery.md#clean-reflash-the-device) Pobieranie [jest](https://aka.ms/hololens2download) aktualne i udostępnia najnowszą ogólnie dostępną kompilację.
...
- Upload wstrzyma się, jeśli urządzenie zasypia lub jest wyłączone. Aby upewnić się, że oczekujące przekazywanie zostało ukończone, aktywnie używaj urządzenia do momentu, gdy na stronie usługi Ustawienia zostanie odczytany stan "OneDrive jest aktualny" lub dostosuj ustawienia uśpienia usługi **Power &.**
...
- Rozwiązano problem, który mógł spowodować obcięcie tekstu w menu power.
...
>
> Wraz z wprowadzeniem wersji Windows Holographic 21H1 zaprzestajemy comiesięcznych aktualizacji obsługi dla systemu Windows Holographic w wersji **1903.** Dzięki temu możemy skupić się na najnowszych wersjach i nadal dostarczać cenne ulepszenia.
...
Nowa Microsoft Edge ![ikona Microsoft Edge nową.](images/new_edge_logo.png) (reprezentowana przez niebieską i zieloną ikonę wirowania) jest przypięta do witryny menu Start zostanie automatycznie uruchamiana po aktywowaniu linku internetowego.

> [!NOTE]
> Przy pierwszym uruchomieniu nowej aplikacji Microsoft Edge wersji HoloLens 2 ustawienia i dane zostaną zaimportowane ze starszych wersji Microsoft Edge. Jeśli po uruchomieniu nowego Microsoft Edge nadal używasz starszej Microsoft Edge Microsoft Edge, nowe dane nie będą synchronizowane ze starszej wersji Microsoft Edge do nowej Microsoft Edge.
...
Ponieważ nowy Microsoft Edge to natywna aplikacja Win32 z nową warstwą adaptera platformy uniwersalnej systemu Windows umożliwiającą jej uruchamianie na urządzeniach z systemem UWP, takich jak HoloLens 2, niektóre funkcje mogą nie być natychmiast dostępne. W najbliższych miesiącach będziemy obsługiwać nowe scenariusze i funkcje, dlatego sprawdź, czy w tym miejscu znajdują się aktualne informacje.
...
- Ustawienia wyszukiwania: wyszukaj ustawienia ze strony głównej Ustawienia przy użyciu słów kluczowych lub nazwy ustawienia.
- Dźwięk > systemu:
...
1. Po zakończeniu pracy z oknami dialogowymi instrukcji ekran zostanie automatycznie wygaszony do 30%.
    > [!TIP]
    > Jeśli w twoim środowisku masz problemy z wyświetlaniem wygaszanej sceny, możesz ręcznie dostosować poziom jasność HoloLens 2 za pomocą przycisków jasność po lewej stronie urządzenia.
...
| Ochrona prywatności > ruchu                                     | `ms-settings:privacy-motion`                       |
...
- Windows Holographic for Business obsługuje tylko tryby pobierania HTTP i pliki do pobrania z punktu [końcowego Connected Cache Microsoft;](/mem/configmgr/core/plan-design/hierarchy/microsoft-connected-cache) Tryby pobierania równorzędne i przypisania grup nie są obecnie obsługiwane HoloLens równorzędnych.
...
- Klienci korzystający z funkcji AdvancedPhotoCapture lub LowLagPhotoCapture do przechwytywania zdjęć za pomocą HoloLens 2 mogą teraz pobierać kamerę w ciągu maksymalnie 3 sekund po przechwyceniu zdjęcia.
...
W HoloLens 2 pozycje oczu umożliwiają dokładne pozycjonowanie hologramu, wygodne środowisko wyświetlania i lepszą jakość wyświetlania. Pozycje oczu są obliczane wewnętrznie w ramach obliczeń śledzenia wzroku. Jednak wymaga to, aby każdy użytkownik przechodził przez śledzenie wzroku, nawet wtedy, gdy środowisko może nie wymagać danych wejściowych spojrzenia oczu.
...
1. [Flash your HoloLens 2](hololens-insider.md#ffu-download-and-flash-directions) to [19041.1361 or newer build (Flash your HoloLens 2 do 19041.1361 lub nowszej kompilacji).](https://aka.ms/hololens2previewdownload) 
...
- Usunięto potrzebę kart USB-C w celu zmniejszenia wymagań sprzętowych przez włączenie funkcji Autopilot na Wi-Fi podłączonych urządzeniach.
...
- Utrzymuje urządzenia w dzierżawie organizacji, blokując je do dzierżawy nawet za pomocą resetowania urządzenia lub reflash. Z dalszymi zabezpieczeniami przez nie zezwalanie na tworzenie kont w programie za pośrednictwem aprowizowania. 
...
Badacze mają teraz możliwość włączenia trybu badań na urządzeniach HoloLens, aby uzyskać dostęp do wszystkich tych strumieni zewnętrznych czujników nieprzetworzonych obrazów. Tryb badania HoloLens 2 zapewnia również dostęp do odczytów akcelerometru, gyroscope i akcelerometru. Aby chronić prywatność użytkowników, nieprzetworzone obrazy kamer śledzące wzrok nie są dostępne za pośrednictwem trybu badania, ale kierunek spojrzenia jest dostępny za pośrednictwem istniejących interfejsów API.
...
- Rozwiązaliśmy problem, który umożliwiał poprawne skalowanie ikon menu Start w trybie kiosku.
...
- Na większości Windows Mixed Reality wektor kierunku do przodu jest równoległy do ziemi, gdy głowy użytkownika znajduje się w pozycji neutralnej w przód. Jednak wcześniejsze wersje HoloLens 2 wyrównały wektor do paneli wyświetlania, które są odchylone w dół o kilka stopni w stosunku do orientacji idealnej. W nowszej wersji HoloLens 2 rozwiązano ten problem, aby zapewnić spójność semantyczną między kształtami.
...
Windows Funkcja Autopilot HoloLens 2 umożliwia wstępne zarejestrowanie kanału sprzedaży urządzenia HoloLens dzierżawie usługi Intune. Po przybyciu urządzenia są one gotowe do samodzielnego wdrożenia jako urządzenia udostępnione w ramach dzierżawy. Aby skorzystać z zalet samodzielnego wdrażania, urządzenie musi połączyć się z siecią podczas pierwszego ekranu w konfiguracji przy użyciu połączenia USB-C-To-Ethernet.
...
Dowiedz się więcej z przewodnika Windows autopilot for HoloLens 2 evaluation guide (Windows [Autopilot for HoloLens 2 evaluation guide (Ocena rozwiązania Autopilot dla rozwiązania HoloLens 2).](hololens2-autopilot.md)
...
Dostawca [usług konfiguracji (CSP)](/windows/client-management/mdm/configuration-service-provider-reference?redirectedfrom=MSDN) to interfejs do odczytywania, konfigurowania, modyfikowania lub usuwania ustawień konfiguracji na urządzeniu. W tej wersji dodamy obsługę większej liczby zasad w celu zwiększenia kontroli nad wdrożoną kontrolą HoloLens urządzeń. Aby uzyskać listę CSP obsługiwanych przez HoloLens, zobacz [NetworkQoSPolicy CSP (NetworkQoSPolicy CSP).](/windows/client-management/mdm/networkqospolicy-csp)

Nowość w tej wersji:

**Dostawca CSP zasad** 

Dostawca usługi konfiguracji zasad umożliwia przedsiębiorstwu konfigurowanie zasad na Windows urządzeniach. W tej wersji dodaliśmy nowe zasady dla HoloLens, które są wymienione tutaj. Aby dowiedzieć się więcej, zobacz [Policy CSPs supported by HoloLens 2 (CSP zasad obsługiwanych przez HoloLens 2).](/windows/client-management/mdm/policies-supported-by-hololens2)  
