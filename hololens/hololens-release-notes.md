...
- Rozwiązano problem, który Wi-Fi, że nie można było zapisać serwera proxy usługi Wi-Fi, gdy interfejs użytkownika właściwości usługi Wi-Fi został uruchomiony z serwera **Ustawienia -> Network & Internet -> Status -> Properties**.
...
Dodaliśmy nową funkcję do aplikacji HoloLens 2 Ustawienia, która umożliwia klientom automatyczne przekazywanie zdjęć i filmów wideo rzeczywistości mieszanej z folderu Pictures > Camera Roll urządzenia do odpowiedniego folderu OneDrive for work or school. Ta funkcja rozwiązuje lukę w funkcjach w aplikacji [OneDrive](holographic-photos-and-videos.md#share-your-mixed-reality-photos-and-videos) na platformie HoloLens 2, która obsługuje tylko automatyczne przekazywanie z rzutu z aparatu do osobistego konta konto Microsoft klienta (a nie konta służbowego).

**Jak to działa**

- Odwiedź **Ustawienia > System > Mixed Reality Camera,** aby włączyć opcję "Przekazywanie z aparatu".
...
- Upload wstrzyma się, jeśli urządzenie zasypia lub jest wyłączone. Aby upewnić się, że oczekujące przekazywanie zostało ukończone, aktywnie używaj urządzenia do momentu, gdy na stronie usługi Ustawienia zostanie odczytany stan "OneDrive jest aktualny" lub dostosuj ustawienia usługi **Power & uśpienia.**
...
[Nowe identyfikatory AUMID dla nowych aplikacji w trybie kiosku](#use-the-new-settings-and-edge-apps-in-kiosk-modes)  | Identyfikatory AUMID dla nowych Ustawienia i edge. | Administrator IT |
...
W tej wersji wprowadzamy nową wersję aplikacji Ustawienia aplikacji. Nowa aplikacja Ustawienia zawiera nowe funkcje i rozwinięte ustawienia dla usługi HoloLens 2 w następujących obszarach: Dźwięk, Power & uśpienia, Sieć & Internet, Aplikacje, Konta, Ułatwienia dostępu i inne.
...
To nowe ustawienie umożliwia wybranie alternatywnego profilu koloru dla ekranu HoloLens 2. Może to pomóc w dokładniejszej wyświetlaniu kolorów, szczególnie na niższych poziomach jasność ekranu. Kolorowanie wyświetlania można znaleźć w Ustawienia aplikacji na stronie >.
...
1. Wybierz przyciski od 1 do 6, aby natychmiast wypróbować każdy profil kolorów i znaleźć taki, który wygląda najlepiej dla Twoich oczu (zwykle oznacza to, że profil, który pomaga scenie wyglądać na najbardziej neutralną, ze wzorcem skali szarości i wyglądem oczu zgodnie z oczekiwaniami).
    ...
    ![Wyświetl scenę odwzorowania kolorów.](images/color-cal-ui.png)
...
6. Odrzuć menu, dotykając w innym miejscu lub zamykając menu Start za pomocą gestu Start.

#### <a name="update-indicator"></a>Wskaźnik aktualizacji

Gdy aktualizacja jest dostępna, ikona wielokropka zostanie zabłysła, aby wskazać, że ponowne uruchomienie zainstaluje aktualizację Opcje menu również zmienią się, aby odzwierciedlić obecność aktualizacji.<br/><br/>
...
>
> Wybranie **opcji Domyślna** komunikacja oznacza, że zewnętrzny mikrofon będzie używany w uchęce Zdalnej pomocy i innych aplikacjach komunikacyjnych, ale macierz mikrofonu HoloLens nadal może być używana do innych zadań.
...
|------------------------------------------------------|--------------------------------------------------|
| Funkcje > Apps & Aplikacje                               | `ms-settings:appsfeatures`                         |
...
### <a name="configuring-fallback-diagnostics-via-settings-app"></a>Konfigurowanie diagnostyki powrotu za pośrednictwem Ustawienia aplikacji

Teraz w Ustawienia aplikacji użytkownik może skonfigurować zachowanie diagnostyki [rezerwowej](hololens-diagnostic-logs.md). W aplikacji Ustawienia przejdź do strony **Rozwiązywanie**  ->  **problemów z prywatnością,** aby skonfigurować to ustawienie.
...
Dzięki tej HoloLens program Windows Holographic for Business ustawienia optymalizacji dostarczania w celu zmniejszenia zużycia przepustowości w przypadku pobierania z HoloLens urządzeń. Pełniejsze opisy tej funkcji wraz z zalecaną konfiguracją sieci są dostępne tutaj: Optymalizacja dostarczania [dla Windows 10 aktualizacji.](/windows/deployment/update/waas-delivery-optimization)
...
- Windows Holographic for Business obsługuje tylko tryby pobierania http i pobieranie z punktu [końcowego usługi Microsoft Connected Cache ;](/mem/configmgr/core/plan-design/hierarchy/microsoft-connected-cache) Równorzędne tryby pobierania i przypisania grup nie są obecnie obsługiwane HoloLens równorzędnych.
...
### <a name="whats-new-in-windows-holographic-version-20h2"></a>Co nowego w wersji Windows Holographic w wersji 20H2  
...
Aby uzyskać więcej informacji na temat stosowania pakietów aprowizowania podczas OOBE, odwiedź HoloLens [dokumentację aprowingu.](hololens-provisioning.md#apply-a-provisioning-package-to-hololens-during-setup)
...
Gdy węzeł RequireNetworkInOOBE dla węzła TenantLockdown CSP jest ustawiony na wartość true HoloLens 2, następujące operacje są niedozwolone w OOBE: 
...
1. Upewnij się, HoloLens 2 urządzenie należy do grupy utworzonej w poprzednim kroku i wyzwolić synchronizację.  
...
1. Utwórz niestandardowy profil konfiguracji urządzenia oparty na URI OMA i określ wartość false dla wartości RequireNetworkInOOBE, jak pokazano poniżej. Wartość OMA-URI powinna mieć wartość ./Vendor/MSFT/TenantLockdown/RequireNetworkInOOBE
...
Dla urządzeń z systemem HoloLens 2 na platformie Windows Holographic w wersji 20H2 zostały utworzone nowe zasady rzeczywistości mieszanej. Nowe ustawienia z możliwością kontrolowania obejmują: ustawianie jasność, ustawianie głośności, wyłączanie rejestrowania dźwięku w przechwytywaniu rzeczywistości mieszanej, ustawianie czasu zbierania danych diagnostycznych i pamięć podręczną członkostwa w grupach usługi AAD.  
...
- Włączono kioski w trybie offline do użytku z grupami usługi AAD przez maksymalnie 60 dni.
...
> [!NOTE]
> W odniesieniu do [funkcji RemoteLock](/windows/client-management/mdm/remotelock-csp)program HoloLens tylko konfigurację ./Vendor/MSFT/RemoteLock/Lock. Konfiguracje związane z kodem PIN, takie jak resetowanie i odzyskiwanie, nie są obsługiwane.
...
Badacze mają teraz możliwość włączenia trybu badań na urządzeniach HoloLens, aby uzyskać dostęp do wszystkich tych strumieni czujników nieprzetworzonych obrazów dostępnych z zewnątrz. Tryb badań dla HoloLens 2 zapewnia również dostęp do akcelerometru, gyroscope i odczytów krokomierza. Aby chronić prywatność użytkowników, nieprzetworzone obrazy kamer śledzące wzrok nie są dostępne za pośrednictwem trybu badania, ale kierunek spojrzenia jest dostępny za pośrednictwem istniejących interfejsów API.
...
- Rozwiązano problem, który powodował, HoloLens, które zmieniają format pikseli w celu renderowania czarnego w HoloLens Emulator.
- Rozwiązano problem z uruchamianiem aplikacji Photos podczas początkowych uruchomień po aktualizacji z wersji 1903.
...
Dostawca usługi konfiguracji zasad umożliwia przedsiębiorstwu konfigurowanie zasad na urządzeniach Windows zasad. W tej wersji dodaliśmy nowe zasady dla HoloLens, które są wymienione tutaj. Aby dowiedzieć się więcej, zobacz [Policy CSPs supported by HoloLens 2 (CSP zasad obsługiwanych przez HoloLens 2).](/windows/client-management/mdm/policies-supported-by-hololens2)  
...
- **Zmniejszenie przypadkowych nacięć w powietrzu:** Ulepszyliśmy wykrywanie gestu naciśnięcia w powietrzu. Teraz istnieje mniej przypadkowych aktywacji w kilku typowych scenariuszach, takich jak po upuszczenie rąk na boki.
...
- Rozwiązano problem, który dotyczył sytuacji, w której podsystem śledzenia oczu nie zainicjował lub w pewnych warunkach nie przeprowadzał awarii.
...
- Deweloperzy unreal mogą teraz używać strony Widok 3D w Portal urządzeń do testowania i debugowania swoich aplikacji.
