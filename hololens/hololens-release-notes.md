...
ms.locfileid: "127985645"
---
# <a name="hololens-2-release-notes"></a>HoloLens wersji 2

Aby zapewnić produktywne środowisko pracy z urządzeniami HoloLens, kontynuujemy wydanie funkcji, usterek i aktualizacji zabezpieczeń. Na tej stronie możesz zobaczyć, co nowego w poszczególnych HoloLens miesiącu. Aby uzyskać najnowszą aktualizację HoloLens 2, możesz sprawdzić aktualizacje i ręcznie zaktualizować lub pobrać pełną aktualizację flash (FFU) w celu flashować urządzenie za pomocą narzędzia [Advanced Recovery Companion.](hololens-recovery.md#clean-reflash-the-device) [](hololens-update-hololens.md#check-for-updates-and-manually-update) Pobieranie [jest](https://aka.ms/hololens2download) na bieżąco i zapewnia najnowszą ogólnie dostępną kompilację.
...
- Rozwiązano problem, Wi-Fi nie można było zapisać serwera proxy usługi Wi-Fi, gdy interfejs użytkownika właściwości usługi Wi-Fi został uruchomiony z usługi **Ustawienia > Network & Internet > status > Properties**(
...
- Upload wstrzyma się, jeśli urządzenie zasypia lub jest wyłączone. Aby upewnić się, że oczekujące przekazania zostaną ukończone, aktywnie używaj urządzenia do momentu, gdy na stronie usługi Ustawienia zostanie odczytany OneDrive jest aktualny" lub dostosuj ustawienia usługi **Power & uśpienia.**
...
- Rozwiązano problem, który mógł spowodować obcięcie tekstu w menu usługi Power.
...
[Nowa Ustawienia aplikacji](#new-settings-app) | Starsza Ustawienia jest zastępowana przez zaktualizowaną wersję nowymi funkcjami i ustawieniami. | Użytkownik końcowy |
[Wyświetlanie koloru na kolor](#display-color-calibration) | Wybierz alternatywny profil kolorów dla ekranu HoloLens 2. | Użytkownik końcowy |
...
[Nowe ustawieniaURI dla widoczności Ustawienia strony](#new-settings-uris-for-page-settings-visibility) | Ponad 20 nowych interfejsów USTAWIENIA/PageVisibilityList. | Administrator IT |
...
> [!NOTE]
> Przy pierwszym uruchomieniu nowej Microsoft Edge wersji HoloLens 2 ustawienia i dane zostaną zaimportowane ze starszej wersji Microsoft Edge. Jeśli po uruchomieniu nowego Microsoft Edge nadal używasz starszej wersji Microsoft Edge, nowe dane nie będą synchronizowane ze starszej wersji Microsoft Edge do nowej Microsoft Edge.
...
> [!IMPORTANT]
> Ze względu na dużą ilość zasad przeglądarki obsługiwanych przez nowe Microsoft Edge, nasz zespół nie może zagwarantować, że każda nowa zasada będzie HoloLens 2. Jednak przetestowaliśmy i potwierdziliśmy, że nowe Microsoft Edge równoważne każdej starszej wersji zasad Microsoft Edge wcześniej obsługiwanych w programie HoloLens 2 działają zgodnie z oczekiwaniami. Zobacz [Starsza wersja Microsoft Edge, Microsoft Edge mapowania](/deployedge/microsoft-edge-policy-map-legacy-to-newedge) zasad, aby znaleźć nowe Microsoft Edge równoważne każdej starszej Microsoft Edge zasad przeglądarki, których używano w programie HoloLens 2.
>
...
Ponieważ nowy Microsoft Edge to natywna aplikacja Win32 z nową warstwą adaptera platformy UWP umożliwiającą jej uruchamianie na urządzeniach z systemem UWP, takich jak HoloLens 2, niektóre funkcje mogą nie być natychmiast dostępne. W najbliższych miesiącach będziemy obsługiwać nowe scenariusze i funkcje, dlatego sprawdź, czy w tym miejscu znajdują się aktualne informacje.
...
- Interakcja z aktywnymi Microsoft Edge windows może spowodować nieoczekiwane nieaktywność innych okien aplikacji 2D. Te okna można ponownie uaktywnić, ponownie korzystając z nich.
...
**Instalowanie za pośrednictwem komputera Windows Portal urządzeń [(wymaga](/windows/mixed-reality/develop/platform-capabilities-and-apis/using-the-windows-device-portal#setting-up-hololens-to-use-windows-device-portal) włączonego trybu dewelopera na HoloLens 2)**
...
#### <a name="managing-endpoints-for-the-new-microsoft-edge"></a>Zarządzanie punktami końcowymi dla nowej Microsoft Edge
...
W tej wersji wprowadzamy nową wersję aplikacji Ustawienia aplikacji. Nowa aplikacja Ustawienia zawiera nowe funkcje i rozszerzone ustawienia dla usługi HoloLens 2 w następujących obszarach: Dźwięk, Power & uśpienia, Sieć & Internet, Aplikacje, Konta, Ułatwienia dostępu i inne.
...
#### <a name="display-color-calibration"></a>Wyświetlanie koloru na kolor

To nowe ustawienie umożliwia wybranie alternatywnego profilu koloru dla wyświetlanego HoloLens 2. Może to ułatwić wyświetlanie kolorów bardziej precyzyjnych, szczególnie na niższych poziomach jasność ekranu. Kolor wyświetlania można znaleźć w aplikacji Ustawienia na stronie > firmy.
...
1. Wybierz przyciski 1–6, aby natychmiast wypróbować każdy profil kolorów, i znajdź taki, który wygląda najlepiej dla Twoich oczu (zazwyczaj oznacza to, że profil, który pomaga scenie wyglądać najbardziej neutralnie, ze wzorcem skali szarości i kolorem wygląda zgodnie z oczekiwaniami).
...
> - Jeśli ktoś na urządzeniu wcześniej użył tego ustawienia do zmiany profilów kolorów, data/godzina ostatniej zmiany zostanie odzwierciedlona na Ustawienia kolorów
...
Jeśli niestandardowy profil kolorów zapisany na urządzeniu HoloLens 2 jest niezadowolony, możesz przywrócić oryginalny profil koloru urządzenia:

1. Uruchom aplikację **Ustawienia** i przejdź do > **System .**
1. W **obszarze Wyświetl kolor kolor** wybierz przycisk **Przywróć domyślny profil** kolorów.
1. Po otworeniu okna dialogowego wybierz **pozycję** Uruchom ponownie, jeśli wszystko jest gotowe do ponownego HoloLens 2 i zastosowania zmian.
...
Teraz w Windows kompilacji użytkownicy mogą ręcznie dostosować poziom woluminu każdej aplikacji. Dzięki temu użytkownicy mogą lepiej skupić się na aplikacjach, których potrzebują, lub lepiej słyszeć w przypadku korzystania z wielu aplikacji. Może to na przykład wymagać wyłączenia woluminu jednej aplikacji podczas rozmowy z inną osobą w celu uzyskania pomocy zdalnej w innej.
...
>
> Wybranie **opcji Domyślna** komunikacja oznacza, że zewnętrzny mikrofon będzie używany w uchęce zdalnej pomocy i innych aplikacjach komunikacyjnych, ale HoloLens macierz mikrofonu może być nadal używana do innych zadań.
...
Niestety, Bluetooth mikrofony nie są obecnie obsługiwane w HoloLens 2.
...
Począwszy od tej Windows wersji, środowisko kiosku będzie w razie awarii w trybie kiosku grupy usługi AAD awarii w globalnej konfiguracji kiosku (jeśli jest obecna).
...
>
> Jedna strona ustawień jest przestarzała w tych nowszej kompilacji. Stara strona **serwera & proxy**  >  **sieci** nie jest już dostępna jako ustawienie globalne. Nowe ustawienia serwera proxy dla połączenia można znaleźć w obszarze Właściwości sieci **& Sieci**  >  **Wi-Fi** lub Właściwości  >   sieci & **Internet**  >    >  **Ethernet.**
...
### <a name="configuring-fallback-diagnostics-via-settings-app"></a>Konfigurowanie diagnostyki rezerwowej za pośrednictwem Ustawienia aplikacji
...
Udostępniaj rzeczy w pobliżu Windows 10 urządzeń, w tym komputerów i innych urządzeń HoloLens 2. Możesz wypróbować to w **programie Ustawienia**  >  **System**  >  **Shared Experiences,** aby udostępniać pliki lub adresy URL z HoloLens na komputerze. Aby uzyskać więcej informacji, przeczytaj więcej na temat sposobu udostępniania rzeczy [pobliskim urządzeniem w Windows 10](https://support.microsoft.com/windows/share-things-with-nearby-devices-in-windows-10-0efbfe40-e3e2-581b-13f4-1a0e9936c2d9).

Tą funkcją można zarządzać za [pośrednictwem elementu Connectivity/AllowConnectedDevices.](/windows/client-management/mdm/policy-csp-connectivity#connectivity-allowconnecteddevices)

### <a name="new-os-diagnostic-traces"></a>Nowe ślady diagnostyczne systemu operacyjnego

Oprócz poprzednich funkcji rozwiązywania problemów w aplikacji Ustawienia dodano nowe narzędzie do rozwiązywania problemów wraz z dodaniem nowej aplikacji Ustawienia aktualizacji systemu operacyjnego. Przejdź do **Ustawienia**  >  **Rozwiązania problemów z &amp; zabezpieczeniami aktualizacji**  >  **Windows**  >  **aktualizacji i** wybierz pozycję **Uruchom.** Dzięki temu można zbierać ślady podczas odtwarzania problemu z aktualizacjami systemu operacyjnego, aby lepiej pomóc w rozwiązywaniu problemów z pomocą techniczną lub IT.

### <a name="delivery-optimization-preview"></a>Optymalizacja dostarczania zapoznawcza

Dzięki tej HoloLens program Windows Holographic for Business ustawienia optymalizacji dostarczania w celu zmniejszenia zużycia przepustowości w przypadku pobierania z HoloLens urządzeń. Pełniejsze opisy tej funkcji wraz z zalecaną konfiguracją sieci są dostępne tutaj: Optymalizacja dostarczania [do Windows 10 aktualizacji.](/windows/deployment/update/waas-delivery-optimization)
...
- HoloLens w tej wersji zapoznawczej obsługa jest ograniczona tylko do aktualizacji systemu operacyjnego.
...
- Rozwiązywanie problemów będzie wymagało diagnostyki na serwerze Connected Cache lub zebrania śladu na serwerze HoloLens w HoloLens za pośrednictwem usługi **Ustawienia**  >  **Update & Security**  >   **Troubleshooting** Windows  >   **Update.**
...
Jeśli obecnie używasz funkcji [Widoczność Ustawienia](settings-uri-list.md) strony, możesz chcieć wprowadzić zmiany istniejących URI, które zostały dozwolone lub zablokowane.
...
✔️[ślady diagnostyczne systemu](#new-os-diagnostic-traces)operacyjnego: zbieranie dzienników związanych z aktualizacjami systemu operacyjnego.
...
- Zwiększa odporność aktualizacji systemu operacyjnego na wstępnie zainstalowanych aplikacjach, takich jak Microsoft Edge.
...
- Ta miesięczna aktualizacja jakości nie zawiera żadnych owalnych zmian. **Ta kompilacja nie będzie już otrzymywać comiesięcznych aktualizacji usługi.** Zachęcamy do wypróbowania naszej najnowszej kompilacji, Windows Holographic, wersja 21H1.
...
- Rozwiązuje problem, który Ustawienia awarii aplikacji podczas próby zmiany hasła dla konta lokalnego.
...
- Klienci korzystający z funkcji AdvancedPhotoCapture lub LowLagPhotoCapture do przechwytywania zdjęć za pomocą HoloLens 2 mogą teraz pobierać kamery pozy do 3 sekund po przechwyceniu zdjęcia.
- Poprawka rozsyłania przecieku pamięci w usłudze Portal urządzeń Service, który powodował zwiększone użycie pamięci przez usługę, co powodowało niepowodzenie przydzielania pamięci przez inne aplikacje.
...
| [Tryb badań](hololens-release-notes.md#research-mode) | Korzystanie z trybu badań w HoloLens 2. |
...
**Funkcja Automatycznego położenia oka (AEP)** umożliwia tym scenariuszom bez interakcji obliczanie pozycji oka dla użytkownika. Automatyczne położenie oka automatycznie rozpoczyna pracę w tle od momentu, gdy użytkownik umieści urządzenie w urządzeniu. Jeśli użytkownik nie ma wcześniejszego śledzenia wzroku, funkcja autowyjadowania okiem zacznie dostarczać użytkownikowi pozycje oczu do systemu wyświetlania po czasie przetwarzania 20–30 sekund. Dane użytkownika nie są utrwalane na urządzeniu, dlatego ten proces jest powtarzany, jeśli użytkownik wystartuje i ponownie uruchomi urządzenie lub jeśli urządzenie zostanie ponownie uruchomiony lub wznowiony po uśpieniu.
...
Wszystkie inne zachowania systemu będą podobne do tego, gdy bieżący użytkownik nie ma aktywnego śledzenia wzroku. Na przykład gest One-handed Start nie zostanie włączony. Nie będzie żadnych zmian w zakresie out-of-box-experience na potrzeby konfiguracji początkowej.

W przypadku doświadczeń, które wymagają danych o oku lub bardzo precyzyjnego pozycjonowania hologramów, zalecamy użytkownikom nieskalicznym uruchamianie śledzenia oczu. Jest on dostępny z monitu o śledzenie oczu lub przez uruchomienie aplikacji Ustawienia z menu Start, a następnie wybranie pozycji System > Wymusienie > Oka w > uruchomienie **oka.**
...
Przed wydaniem tej wersji użytkownicy musieli ręcznie uruchomić ekran aprowizowania podczas OOBE, aby aprowizować przy użyciu kombinacji przycisku. Teraz użytkownicy mogą pominąć kombinację przycisków przy użyciu pakietu aprowizowania na dysku pamięci MASOWEJ USB.

1. Podłącz dysk USB za pomocą pakietu aprowizowania podczas pierwszej interakcji OOBE
...
Gdy pojawi się ekran główny inicjowania obsługi administracyjnej, program OOBE odliczy 10 sekund, zanim automatycznie rozpocznie stosowanie wszystkich pakietów aprowizowania. Użytkownicy mogą nadal [potwierdzić lub anulować w](hololens-provisioning.md#auto-confirm-provisioning-packages-in-oobe) ciągu tych 10 sekund po zweryfikowaniu oczekiwanych pakietów.
...
Urządzenie jest teraz skonfigurowane i zostanie na jego stronie [wyświetlany ekran Aprowizowanie powiodło się.](hololens-provisioning.md#automatic-provisioning-without-using-ui)

### <a name="using-autopilot-with-wi-fi-connection"></a>Używanie rozwiązania Autopilot z Wi-Fi połączenia

- Usunięto potrzebę adapterów USB-C do sieci Ethernet, co zmniejsza wymagania sprzętowe, włączając funkcję Autopilot na Wi-Fi połączonych urządzeniach.

Teraz podczas OOBE po połączeniu HoloLens 2 z siecią Wi-Fi system OOBE sprawdzi profil rozwiązania Autopilot dla urządzenia. Jeśli zostanie znaleziony, zostanie on użyty do ukończenia pozostałej części przepływu dołączania do i rejestracji w UAD. Innymi słowy, używanie sieci Ethernet do portu USB C lub Wi-Fi z adapterem USB-C nie jest już wymagane, jednak nadal działają, jeśli zostały podane na początku OOBE. Dowiedz się więcej na [temat rozwiązania Autopilot HoloLens 2 urządzeń.](hololens2-autopilot.md)
...
Gdy węzeł RequireNetworkInOOBE dla węzła TenantLockdown CSP jest ustawiony na wartość true w HoloLens 2, następujące operacje są niedozwolone w OOBE:
...
#### <a name="how-to-unset-tenantlockdowns-requirenetworkinoobe-on-hololens-2-using-intune"></a>Jak unset TenantLockdown requireNetworkInOOBE w usłudze HoloLens 2 przy użyciu usługi Intune?
...
Te informacje można teraz znaleźć obok pozostałej części rozwiązania Autopilot w obszarze [Tenantlockdown CSP (Dzierżawa)CSP (CSP) i Autopilot (Autopilot).](hololens2-autopilot.md#tenant-lockdown-csp-and-autopilot)
...
Ta nowa funkcja umożliwia administratorowi IT skonfigurowanie urządzenia z systemem HoloLens 2 dla trybu kiosku z wieloma aplikacjami, który ma zastosowanie na poziomie systemu, nie ma koligacji z żadną tożsamością w systemie i ma zastosowanie do wszystkich osób, które się na nim pojawiają. Przeczytaj szczegółowo o tej nowej funkcji w trybie [HoloLens kiosku.](hololens-kiosk.md)
...
Wcześniej w przypadku napotkania błędów podczas stosowania trybu kiosku HoloLens do pokazania wszystkich aplikacji w menu Start. Teraz w Windows Holographic w wersji 20H2 w przypadku awarii żadne aplikacje nie będą wyświetlane w menu Start, jak pokazano poniżej:
...
Dla urządzeń z systemem HoloLens 2 na platformie Windows Holographic w wersji 20H2 zostały utworzone nowe zasady rzeczywistości mieszanej. Nowe ustawienia z możliwością kontrolowania obejmują: ustawienie jasność, ustawienie głośności, wyłączenie rejestrowania dźwięku w przechwytywaniach rzeczywistości mieszanej, ustawienie czasu zbierania danych diagnostycznych i pamięć podręczną członkostwa w grupie usługi AAD.  
...
| MixedReality\MicrophoneDisabled                    | Wyłącza mikrofon, aby nie było możliwe nagrywanie dźwięku na HoloLens 2.                      | 1 Tak, 0 Nie (ustawienie domyślne)                                                |
...
1. Teraz użytkownik 1 usługi Azure AD może przeczekać HoloLens tryb offline i używać go w trybie kiosku, o ile wartość zasad umożliwia X liczbę dni.
...
Nowo włączone zasady, które umożliwiają obsługę większej liczby opcji zarządzania HoloLens 2 urządzeń.
...
> [!NOTE]
> W odniesieniu do [remoteLock](/windows/client-management/mdm/remotelock-csp)HoloLens będzie obsługiwać tylko konfigurację ./Vendor/MSFT/RemoteLock/Lock. Konfiguracje dotyczące numeru PIN, takie jak resetowanie i odzyskiwanie, nie są obsługiwane.

### <a name="new-power-policies-for-hololens-2"></a>Nowe zasady zasilania dla HoloLens 2

- Więcej opcji w przypadku uśpienia HoloLens blokady za pośrednictwem zasad zasilania.
...
Te zasady aktualizacji są teraz włączone na HoloLens 2 urządzeniach:
...
Szczegółowe informacje na temat tych zasad aktualizacji i sposobu ich używania na urządzeniach HoloLens można znaleźć tutaj w tece Zarządzanie HoloLens [aktualizacjami.](hololens-updates.md)
...
Włączyliśmy zasady, które umożliwiają administratorom IT uniemożliwić widoczność lub dostępność określonych stron w aplikacji systemowej Ustawienia lub na korzystanie z nich dla wszystkich stron z wyjątkiem określonych. Aby dowiedzieć się, jak w pełni dostosować tę funkcję, kliknij poniższy link.

- [PageVisibilityList](/windows/client-management/mdm/policy-csp-settings#settings-pagevisibilitylist)

Aby dowiedzieć się, które ustawienia strony można dostosować w HoloLens 2, odwiedź naszą Ustawienia [URIs](settings-uri-list.md).
...
W trybie badań komputer HoloLens 2 staje się narzędziem do badania obrazów. W porównaniu z poprzednimi wersjami tryb badań dla HoloLens 2 ma następujące zalety:
...
Badacze mają teraz możliwość włączenia trybu badań na urządzeniach HoloLens, aby uzyskać dostęp do wszystkich tych strumieni zewnętrznych czujników nieprzetworzonych obrazów. Tryb badań dla HoloLens 2 zapewnia również dostęp do akcelerometru, żyrometru i akcelerometrów. Aby chronić prywatność użytkowników, nieprzetworzone obrazy kamer śledzące wzrok nie są dostępne za pośrednictwem trybu badania, ale kierunek spojrzenia jest dostępny za pośrednictwem istniejących interfejsów API.

Zapoznaj się z [dokumentacją trybu badań,](/windows/mixed-reality/research-mode) aby uzyskać więcej szczegółów technicznych.

### <a name="recording-length-increased"></a>Zwiększona długość rejestrowania
...
Pełne wymagania można znaleźć w dokumentacji dotyczącej [zdjęć i filmów holograficznych.](holographic-photos-and-videos.md#maximum-recording-length)

### <a name="improvements-and-fixes-in-the-windows-holographic-version-20h2-update"></a>Ulepszenia i poprawki w wersji Windows Holographic w wersji 20H2:
...
- Rozwiązano problem, który mógł dotyczyć sytuacji, w której użytkownicy nie mogli aprowizować profilów sieci VPN za pośrednictwem pakietów aprowizowania.
...
- Rozwiązano problem uniemożliwiający wyświetlanie urządzenia HoloLens w uciece Eksplorator plików za pośrednictwem protokołu MTP (Media Transfer Protocol), gdy urządzenie jest ustawione jako kiosk z jedną [aplikacją.](hololens-kiosk.md) Pamiętaj, że usługę MTP (i ogólnie połączenie USB) można wyłączyć przy użyciu [zasad AllowUSBConnection.](/windows/client-management/mdm/policy-csp-connectivity#connectivity-allowusbconnection)
- Rozwiązano problem, który umożliwiał poprawne skalowanie ikon menu Start w trybie kiosku.
- Rozwiązano problem spowodowany zakłócanie buforowania HTTP w trybie kiosku skierowanym do grup usługi Azure AD.
...
Ta comiesięczna aktualizacja jakości nie zawiera żadnych notowalnych zmian. Zachęcamy do wypróbowania najnowszej kompilacji wydania funkcji Windows Holographic w wersji 20H2.
...
- Rozwiązuje problem uniemożliwiający debugowanie Visual Studio, gdy element SupportsMultipleInstances="true" występuje w pliku appxmanifest.
- Ta wersja zawiera poprawkę wykrywania serwera proxy NCSI w celu rozwiązania problemu z nieudanym wykrywaniem Internetu za pośrednictwem serwera proxy sieci. Usługa NCSI może używać serwera proxy komputera i serwera proxy dla profilu do wykrywania łączności z Internetem. Serwer proxy dla użytkownika będzie obsługiwany przez interfejs NCSI w przyszłej wersji.
- Na większości Windows Mixed Reality wektor kierunku do przodu jest równoległy do ziemi, gdy głowy użytkownika znajduje się w pozycji neutralnej w przód. Jednak wcześniejsze wersje HoloLens 2 wyrównały wektor do paneli wyświetlania, który jest odchylony w dół o kilka stopni w stosunku do orientacji idealnej. W nowszej wersji HoloLens 2 rozwiązano ten problem, aby zapewnić spójność semantyczną między kształtami.
...
- Ustawienia nie będzie śledzić użytkownika w doświadczeniach rejestracji irysów ani śledzenia oczu.
- Usunięto usterkę, która oznaczała, że zastosowanie pakietu aprowizowania podczas procesu OOBE, który zmienia nazwę urządzenia i wykonuje inne akcje (takie jak nawiązywanie połączenia z siecią), nie może wykonać innych akcji po ponownym uruchomieniu urządzenia z powodu zmiany nazwy.
...
- Zwiększono niezawodność uruchamiania aplikacji po aktualizacjach systemu operacyjnego.
...
- Rozwiązano problem, który powodował, że HoloLens, które zmieniają format pikseli na czarny, w HoloLens emulatorze.
...
- Rozwiązano problem, który powodował, HoloLens, które zmieniają format pikseli na czarny w HoloLens Emulator.
- Rozwiązano problem z uruchamianiem aplikacji Zdjęcia podczas początkowych uruchomień po aktualizacji z wersji 1903.
...
|       Ulepszona aprowzowanie                      |          Bezproblemowe stosowanie pakietu aprowizowania z dysku USB do urządzenia HoloLens                              |
...
|       Ulepszenia śledzenia rąk                 |          Ulepszenia śledzenia rąk sprawiają, że przyciski i interakcje planszy 2D są dokładniejsze                        |
...
Dowiedz się więcej z [przewodnika Windows autopilot dla HoloLens 2.](hololens2-autopilot.md)
...
Pakiety aprowizowania umożliwiają HoloLens konfiguracji za pomocą pliku konfiguracji, a nie HoloLens za pośrednictwem wstępnie dostępnego typu. Wcześniej pakiety aprowizowania były kopiowane do HoloLens pamięci wewnętrznej. Teraz mogą one być na dysku USB, dzięki czemu można je łatwiej ponownie używać na wielu urządzeniach HoloLens i można aprowizować urządzenia równolegle. Pakiety aprowizowania obsługują teraz również pole do rejestrowania w zarządzaniu urządzeniami, dzięki czemu po aprowizeniu nie ma żadnej ręcznej konfiguracji.

Aby ją wypróbować:

1. Pobierz najnowszą wersję programu Windows Configuration Designer z Windows magazynu na komputer.
1. Wybierz **pozycję Aprowizuj HoloLens urządzeń**  >  **Aprowizuj HoloLens 2 urządzenia.**
1. Skompilowanie profilu konfiguracji. Następnie skopiuj wszystkie pliki, które zostały utworzone na urządzenie magazynujące USB-C.
1. Podłącz urządzenie USB-C do dowolnych flashowanych HoloLens. Następnie naciśnij przyciski **zasilania regulacji głośności,**  +   aby zastosować pakiet aprowizowania.
...
Dostawca [usług konfiguracji (CSP)](/windows/client-management/mdm/configuration-service-provider-reference?redirectedfrom=MSDN) to interfejs do odczytywania, konfigurowania, modyfikowania lub usuwania ustawień konfiguracji na urządzeniu. W tej wersji dodamy obsługę większej liczby zasad w celu zwiększenia kontroli nad wdrożonym HoloLens urządzeń. Aby uzyskać listę CSP obsługiwanych przez HoloLens, zobacz [NetworkQoSPolicy CSP (NetworkQoSPolicy CSP).](/windows/client-management/mdm/networkqospolicy-csp)

Nowość w tej wersji:

**Dostawca CSP zasad** 

Dostawca usługi konfiguracji zasad umożliwia przedsiębiorstwu konfigurowanie zasad na Windows urządzeń. W tej wersji dodaliśmy nowe zasady dla HoloLens, które są wymienione tutaj. Aby dowiedzieć się więcej, zobacz [Policy CSPs supported by HoloLens 2](/windows/client-management/mdm/policies-supported-by-hololens2)(Zasady CSP obsługiwane przez HoloLens 2).  
...
Wiele Windows obsługuje teraz tryb ciemny i jasny. HoloLens 2 użytkownicy mogą wybrać tryb domyślny dla aplikacji, które obsługują oba te tryby. Po aktualizacji domyślny tryb aplikacji jest "ciemny", ale możesz łatwo zmienić to ustawienie: Przejdź do Ustawienia Aplikacji Wybierz  >    >    >  **domyślny tryb aplikacji.**
...
- Rozwiązano problem, który pozwalał użytkownikom na eksportowanie dzienników zarządzania urządzeniami przenośnymi za **pośrednictwem Ustawienia**.
...
- Upewniliśmy się, że nakładki powłoki są uwzględniane w przechwytywaniu rzeczywistości mieszanej.
...
- Naprawiono błąd "Klasa interfejsu API IStreamSocketListener WinRT nie jest zarejestrowana" w 32-bitowych aplikacjach ARM.
...
- Poprawiono stabilność hologramów w przechwytywaniu rzeczywistości mieszanej, gdy jest używany algorytm *HolographicDepthReprojectionMethod AutoPlanar.*
- Upewniliśmy się, że układ współrzędnych dołączony do próbki mf głębokości jest zgodny z publiczną dokumentacją.
- Zwiększona produktywność deweloperów dzięki umożliwieniu klientom wklejania dużych ilości tekstu za pośrednictwem portalu urządzeń.
...
- Usunięto losową awarię HUP spowodowaną przez śledzenie rąk, w której użytkownik zauważył, że interfejs użytkownika zawiesza się, a następnie wraca do powłoki po kilku sekundach.
- Ulepszono śledzenie rąk, dzięki czemu podczas przesuwania palcem wskazującym górna część palca jest mniej prawdopodobna do nieoczekiwanego zwinięcia.
...
- Poprawiono stabilność aplikacji wyłącznych podczas pracy z emulatorem HoloLens 2.
...
- Wprowadzono poprawki dotyczące odtwarzania ostatniego etapu (LSR). Ulepszone renderowanie wizualne hologramów w celu bardziej stabilnego i bardziej szczegółowego odwzorowania ich głębokości. Ten objaw będzie bardziej zauważalny po tej aktualizacji, jeśli aplikacje nie ustawią poprawnie głębokości hologramów.
- Poprawiono stabilność aplikacji wyłącznych i nawigację między aplikacjami wyłączności.
- Rozwiązano problem z tym, że przechwytywanie rzeczywistości mieszanej nie mogło nagrać wideo, gdy urządzenie było w stanie wstrzymania przez kilka dni.
...
- Naprawiono funkcjonalność poleceń **głosowych Select** podczas początkowej konfiguracji dla en-CA i en-AU.
- Ulepszona jakość wizualna obiektów umieszczonych daleko w najnowszych wersjach aparatu Unity i Mixed Reality Toolkit (MRTK).
