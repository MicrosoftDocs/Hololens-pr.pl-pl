...
**Znane problemy**
- [Poprawka niejawnego testera — za każdym razem, gdy zasilanie spada do 18%, urządzenie nagle się automatycznie wyłącza](#every-time-the-power-goes-to-18-percent-the-device-suddenly-shuts-down-automatically)
...
- [Dlaczego podczas 0x80180014 Autopilot są 0x80180014?](#why-do-i-see-0x80180014-during-autopilot)
...
- [**Naprawiono** — Portal urządzeń przekazywania/pobierania plików przesłoniło](#device-portal-file-uploaddownload-times-out)
...
> [!NOTE]
> Istnieje poprawka dla tego problemu dostępna w Windows [niejawnych testerów.](hololens-insider.md)
...
- Użytkownicy mogą bezpośrednio uzyskać dostęp OneDrive do aplikacji Microsoft Edge i wchodzić w interakcje z plikami w witrynie internetowej z przeglądarki.
- Użytkownicy mogą zainstalować aplikację OneDrive PWA, aby HoloLens ją pobrać z Microsoft Edge. Umożliwi to użytkownikom wyświetlanie plików na urządzeniu i zarządzanie nimi ponownie. Przeczytaj te instrukcje [dotyczące instalowania aplikacji OneDrive PWA na komputerze HoloLens.](holographic-store-apps.md#install-microsoft-onedrive-pwa-app)

[Powrót do listy](#list)

## <a name="why-do-i-see-0x80180014-during-autopilot"></a>Dlaczego widzę informacje o 0x80180014 funkcji Autopilot?

Ten błąd jest zwykle napotykany podczas resetowania urządzenia i ponownego używania przepływów, w przypadku których HoloLens urządzenie zostało co najmniej raz przejechało przez funkcję Autopilot. Aby rozwiązać ten problem, usuń urządzenie z programu [Microsoft Intune](/mem/autopilot/troubleshoot-device-enrollment#error-code-0x80180014-when-re-enrolling-using-self-deployment-or-pre-provisioning-mode) i zresetuj je ponownie, aby ukończyć przepływ rozwiązania Autopilot.

Aby uzyskać więcej informacji, zapoznaj się z [krokami rozwiązywania problemów na stronie rozwiązania Autopilot.](hololens2-autopilot.md#issue---mdm-enrollment-fails-with-error-0x80180014-error-code-during-autopilot)

## <a name="microsoft-store-error-code-0x80131500"></a>Microsoft Store kod błędu 0x80131500

Niektórzy użytkownicy mogą doświadczyć, Microsoft Store nie działają zgodnie z oczekiwaniami, i zobaczą kod błędu 0x80131500. Jest to problem spowodowany tym, że region ustawiony na stronie HoloLens jest niedostępny w aplikacji Microsoft Store na HoloLens. Jeśli wystąpi błąd z kodem 0x80131500, aby obejść ten problem:
...
Zobacz [tutaj, aby uzyskać informacje o krajach, w których HoloLens 2.](hololens2-purchase.md)

## <a name="microsoft-edge-fails-to-start-the-microphone"></a>Microsoft Edge nie można uruchomić mikrofonu

Gdy użytkownicy używający Microsoft Edge mogą nie uruchomić mikrofonu, w związku z tym nie można używać go do interakcji z usługą Edge w HoloLens. Ten znany problem jest związany z wersją aplikacji Microsoft Edge. Nie przekłasz urządzenia do starszej wersji, ponieważ nie rozwiąże to tego problemu.
...
Bieżąca poprawka jest w wersji 96, która jest dostępna dla użytkowników zarejestrowanych w Microsoft Edge Insiders. Różni się to od rejestrowania urządzenia jako Windows niejawnego testera. Przeczytaj te instrukcje, aby uzyskać szczegółowe [informacje na temat rejestrowania w niejawnym programie edge.](hololens-new-edge.md#microsoft-edge-insider-channels)
...
- Jeśli nie można zapamiętać numeru PIN urządzenia i inne metody uwierzytelniania są niedostępne, użytkownik może użyć [ręcznego trybu reflashingu.](hololens-recovery.md#manual-flashing-mode-procedure)
...
W poprzednich kompilacjach Windows Holographic próba pobrania zablokowanego pliku byłaby stroną błędu HTTP. Na platformie Windows Holographic aktualizacja w wersji 21H1, próba pobrania zablokowanego pliku powoduje, że nic nie dzieje się widoczne — plik nie jest pobierany i nie występuje błąd.

[Powrót do listy](#list)

## <a name="device-portal-file-uploaddownload-times-out"></a>Portal urządzeń czasu przekazywania/pobierania plików
> [!NOTE]
...
To obejście, które ma zastosowanie w Wi-Fi i UsbNcm, polega na wyłączeniu opcji "wymagane" w obszarze "Połączenie SSL". W tym celu przejdź do **Portal urządzeń, System** i wybierz **stronę Preferencje.** W sekcji **Zabezpieczenia urządzeń** znajdź opcję **Połączenie SSL** i usuń zaznaczenie pola wyboru, aby wyłączyć **opcję Wymagane.**

Następnie użytkownik powinien przejść do strony http://, https:// (adres IP), a funkcje, takie jak przekazywanie i pobieranie plików, będą działać.
...
Jest to problem, który ma wpływ na użytkowników, którzy byli w kompilacji niejawnego programu w wersji zapoznawczej, przekrzywilili swój program HoloLens 2 przy użyciu nowej kompilacji niejawnego programu w wersji zapoznawczej, a następnie nie są zarejestrowani w programie niejawnego testera. Jest to znany **problem.**
...
- Reflash urządzenia
    ...
    1. Ręcznie umieść [HoloLens 2](hololens-recovery.md) w trybie flashowania, w pełni przez całkowite zasilanie bez połączenia. Następnie przy przytrzymaniu przycisku Regulacji głośności naciśnij przycisk Zasilania.
...
- Jeśli dioda LED zabłyśnie po naciśnięciu przycisku zasilania, ale nie widzisz niczego na ekranach, wykonaj twarde [resetowanie urządzenia.](hololens-recovery.md#hard-restart-procedure)

Jeśli twój HoloLens jest zamrożony lub nie odpowiada:

- Wyłącz zasilanie HoloLens naciskając przycisk zasilania, aż wszystkie pięć diod LED wyłączy się lub przez 15 sekund, jeśli diody LED nie będą odpowiadać. Aby rozpocząć HoloLens, naciśnij ponownie przycisk zasilania.
...
Jeśli wszystkie wytyczne zostały przestrzegane, a nadal brakuje wskazówek, można wyłączyć monit o iniekcja w Ustawienia. Daj nam znać, składając opinię w [Centrum opinii](hololens-feedback.md).
...
W HoloLens (1. generacji) wbudowanej funkcji rozpoznawania mowy nie można skonfigurować. Jest ona zawsze włączona. W HoloLens 2 można wybrać, czy włączyć zarówno rozpoznawanie mowy, jak i Cortana podczas konfigurowania urządzenia.

Jeśli twój HoloLens 2 nie odpowiada na Twój głos, upewnij się, że rozpoznawanie mowy jest włączone. Przejdź do  >  **tematu Start Ustawienia**  >  **Privacy**  >  **Speech** i włącz **rozpoznawanie mowy.**

[Powrót do listy](#list)

## <a name="hand-input-isnt-working"></a>Ręczne wprowadzanie danych nie działa

Aby upewnić się HoloLens, że twoje ręce są widać, należy je zachować w ramce gestu.  Strona Mixed Reality home udostępnia opinię, która informuje o tym, kiedy twoje ręce są śledzone.  Opinie są inne w różnych wersjach HoloLens:
...
Oto kilka rzeczy do wypróbowania, jeśli nie można połączyć komputera HoloLens z Wi-Fi siecią:

- Upewnij się, Wi-Fi jest włączona. Aby to sprawdzić, użyj gestu Start, a następnie wybierz pozycję **Ustawienia**  >  **Sieci &amp;**  >  **Wi-Fi.** Jeśli Wi-Fi jest wł., spróbuj wyłączyć ją, a następnie ponownie wł.
...
HoloLens 2 obsługuje Bluetooth audio A2DP do odtwarzania stereo. Profil Bluetooth Hands Free, który umożliwia przechwytywanie mikrofonu z urządzenia peryferyjnego Bluetooth nie jest obsługiwane na HoloLens 2.
...
- Bluetooth myszy.
- Kliknij [HoloLens kliknij pozycję](hololens1-clicker.md).

Możesz sparować inne urządzenia Bluetooth HID i EURO ze swoimi HoloLens. Może być jednak trzeba zainstalować odpowiednie aplikacje towarzyszące z Microsoft Store, aby faktycznie korzystać z urządzeń.
