...
**Znane problemy**
- [Za każdym razem, gdy zasilanie wynosi 18%, urządzenie nagle się wyłącza automatycznie](#every-time-the-power-goes-to-18-percent-the-device-suddenly-shuts-down-automatically)
...
## <a name="every-time-the-power-goes-to-18-percent-the-device-suddenly-shuts-down-automatically"></a>Za każdym razem, gdy zasilanie wynosi 18%, urządzenie nagle się wyłącza automatycznie

Istnieje znany problem, który występuje, gdy urządzenie osiągnie poziom baterii 18%, zostanie nieoczekiwanie zamknięte. Jest to problem z oprogramowaniem, a nie problem ze sprzętem lub baterii, dlatego nie wymieniaj w tym celu urządzeń. Jeśli nie masz pewności, czy problem pasuje do tej usterki, zobacz:

1. Upewnij się, że na urządzeniach włączono opcjonalną diagnostykę
1. Odtprodukuj problem
1. Prześlij [Centrum opinii](hololens-feedback.md) problemu
...
Wymaganie wstępne: klienci mogą Microsoft Edge systemu operacyjnego urządzenia i zaktualizować go do Windows Holographic, 21H1 lub nowszej.

Jeśli ten problem występuje, spróbuj wykonać jedną z następujących czynności:

- Użytkownicy mogą bezpośrednio OneDrive dla firm z Microsoft Edge i wchodzić w interakcje z plikami w witrynie internetowej z przeglądarki.
- Użytkownicy mogą zainstalować aplikację OneDrive PWA, aby HoloLens, pobierając ją z Microsoft Edge. Dzięki temu użytkownicy będą ponownie wyświetlać pliki na urządzeniu i zarządzać nimi. Zapoznaj się z tymi [instrukcjami dotyczącymi instalowania OneDrive PWA aplikacji na HoloLens.](holographic-store-apps.md#install-microsoft-onedrive-pwa-app)

[Powrót do listy](#list)

## <a name="why-do-i-see-0x80180014-during-autopilot"></a>Dlaczego widzę informacje 0x80180014 funkcji Autopilot?

Ten błąd jest zwykle napotykany podczas resetowania urządzenia i ponownego używania przepływów, w których urządzenie HoloLens co najmniej raz korzystało z rozwiązania Autopilot. Aby rozwiązać ten problem, usuń urządzenie z Microsoft Intune [i](/mem/autopilot/troubleshoot-device-enrollment#error-code-0x80180014-when-re-enrolling-using-self-deployment-or-pre-provisioning-mode) zresetuj je ponownie, aby ukończyć przepływ rozwiązania Autopilot.
...
Jeśli połączenia są trwać 20 minut i występuje ten problem, spróbuj ponownie uruchomić urządzenie. Ponowne uruchomienie urządzenia między wywołaniami usługi Remote Assist spowoduje odświeżenie urządzenia i jego ponowne uruchomienie w dobrym stanie.
...
> [!NOTE]
> Ten problem został pierwotnie utworzony z myślą o wersji wysyłkowej Microsoft Edge na uwadze. Ten problem można rozwiązać w nowej [Microsoft Edge](hololens-new-edge.md). Jeśli tak nie jest, prosimy o opinię.
...
## <a name="device-portal-file-uploaddownload-times-out"></a>Portal urządzeń umyka czas przekazywania/pobierania plików
> [!NOTE]
...
- Reflash urządzenia
    ...
    1. Umieść urządzenie [HoloLens 2 w trybie flashowania](hololens-recovery.md) ręcznie, w pełni przez całkowite zasilanie bez połączenia. Następnie przytrzymaniu przycisku Volume up naciśnij przycisk Zasilania.
...
- Wyłącz zasilanie HoloLens naciskając przycisk zasilania do momentu wyłączenia wszystkich pięciu diod LED lub na 15 sekund, jeśli diody LED nie będą odpowiadać. Aby uruchomić HoloLens, ponownie naciśnij przycisk zasilania.
...
Niektóre potencjalne przyczyny niepowodzenia awarii systemu to:
...
- Zarost lub grube ramki z okularami, jeśli blokują one wzrok urządzenia
...
- Przenoszenie obiektów w osłodę z drogi (na przykład zarostu)
- Włączanie światła w pomieszczeniu lub wyprowadzanie bezpośredniego światła

Jeśli wszystkie wytyczne zostały przestrzegane i nadal nie można z niej korzystać, możesz wyłączyć monit o monit o Ustawienia. Daj nam znać, klikając opinię w [Centrum opinii](hololens-feedback.md).
...
HoloLens 2 obsługuje profil audio Bluetooth A2DP do odtwarzania stereo. Profil Bluetooth Hands Free, który umożliwia przechwytywanie mikrofonu z urządzenia peryferyjnego Bluetooth nie jest obsługiwany na HoloLens 2.
...
Inne urządzenia HID Bluetooth MOŻNA parować z urządzeniami z systemem HoloLens. Może być jednak trzeba zainstalować odpowiednie aplikacje towarzyszące z Microsoft Store, aby faktycznie korzystać z urządzeń.
