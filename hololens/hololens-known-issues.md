...
Główna przyczyna problemu: dotyczy to użytkowników, którzy używali programu Visual Studio 2015 lub wczesnych wersji programu Visual Studio 2017 do wdrażania i debugowania aplikacji na ich HoloLens, a następnie używali najnowszych wersji programu Visual Studio 2017 lub Visual Studio 2019 z tym samym HoloLens. Nowsze wersje programu Visual Studio nową wersję składnika, ale pliki ze starszej wersji są pozostawiane na urządzeniu, co powoduje niepowodzenie nowszej wersji.  Powoduje to następujący komunikat o błędzie: DEP0100: Upewnij się, że urządzenie docelowe ma włączony tryb dewelopera. Nie można uzyskać licencji dewelopera na program \<ip\> z powodu błędu 80004005.
...
1. Nadaj projektowi nazwę (taką jak "HoloLensDeploymentFix") i upewnij się, że dla struktury ustawiono co najmniej jedną .NET Framework 4.5, a następnie wybierz przycisk **OK.**
...
1. Uruchom plik wykonywalny i podaj adres IP urządzenia jako argument wiersza polecenia. (Jeśli połączenie jest połączone przy użyciu portu USB, możesz użyć adresu 127.0.0.1. W przeciwnym razie użyj adresu IP Wi-Fi urządzenia).  Na przykład "HoloLensDeploymentFix 127.0.0.1".
...
Niektórzy użytkownicy zgłaszali, że zamknięcie zawieszonych aplikacji i uruchomienie innych aplikacji, takich jak Centrum opinii, Przeglądarka 3D lub Photos, rozwiązuje problem dla nich — jednak nie działa to przez 100% czasu.
...
Jeśli urządzenie nadal nie może załadować aplikacji, możesz załadować bezpośrednio wersję programu .NET Native Framework i środowiska uruchomieniowego za pośrednictwem Centrum pobierania, wykonać następujące czynności:
...
- Zobacz [Instalowanie narzędzi,](/windows/mixed-reality/install-the-tools) aby uzyskać najnowszą wersję pakietu Visual Studio zalecaną na HoloLens tworzenia aplikacji.
...
- Kontroler bezprzewodowy Konsoli Xbox S musi zostać zaktualizowany, zanim będzie można go używać z HoloLens. Przed [podjęciem próby sparowania](https://support.xbox.com/xbox-one/accessories/update-controller-for-stereo-headset-adapter) kontrolera z kontrolerem upewnij się, że jesteś na bieżąco HoloLens.
