...
1. Sprawdź, czy profil Wi-Fi ma odpowiednie ustawienia:
    ...
    - Sprawdź nazwę Wi-Fi SSID i sprawdź, czy jest ona taka sama jak ciąg HEX.
    - Upewnij się, że w przypadku protokołu EAP-TLS wartość TrustedRootCA zawiera skrót SHA-1 zaufanego certyfikatu głównego urzędu certyfikacji serwera. Na Windows PC polecenie "certutil.exe -dump cert_file_name" będzie wyświetlać ciąg skrótu SHA-1 certyfikatu.
2. Zbierz przechwytywanie pakietów sieciowych w dziennikach punktu dostępu, kontrolera lub serwera usługi AAA, aby dowiedzieć się, gdzie sesja protokołu EAP kończy się niepowodzeniem.
    - Jeśli tożsamość EAP dostarczana przez HoloLens jest nieoczekiwana, sprawdź, czy tożsamość została poprawnie aprowizowana za pośrednictwem Wi-Fi lub certyfikatu klienta.
    - Jeśli serwer odrzuci certyfikat HoloLens klienta, sprawdź, czy wymagany certyfikat klienta został aprowowany na urządzeniu.
...
3. Jeśli profil przedsiębiorstwa jest aprowowany za pośrednictwem Wi-Fi aprowizowania, rozważ zastosowanie pakietu aprowizowania na komputerze Windows 10 PC. Jeśli na komputerze Windows 10 również nie powiedzie się, postępuj zgodnie z Windows rozwiązywania problemów z uwierzytelnianiem klienta 802.1X.
...
1. Upewnij się, że sieć Wi-Fi jest włączona. Sprawdź za pomocą gestu Start, a następnie wybierz pozycję Ustawienia > Network & Internet > Wi-Fi. Jeśli Wi-Fi jest wł., spróbuj ponownie go wyłączyć i wł.
2. Umieść komputer bliżej routera lub punktu dostępu.
3. Uruchom ponownie Wi-Fi, a następnie uruchom ponownie HoloLens. Spróbuj ponownie nawiązać połączenie.
4. Jeśli żadna z tych czynności nie działa, sprawdź, czy router korzysta z najnowszego oprogramowania układowego. Te informacje można znaleźć w witrynie internetowej producenta.
...
1. Na komputerze — skonfiguruj program Fiddler tak, aby zezwalał komputerom zdalnym na nawiązywanie połączenia.
    1. Przejdź do fiddler Ustawienia -> connections (Połączenia > Fiddler).
...
1. Zmień ustawienie na Ustawienia ręczne.
1. Wprowadź adres IP komputera, na którym zainstalowano program Fiddler.
1. Wprowadź numer portu zanotowyny powyżej (wartość domyślna to 8866).
...
1. Na komputerze — wyeksportuj certyfikat fiddler.
    1. Przejdź do opcji Fiddler Ustawienia -> HTTPS i rozwiń Ustawienia.
...
2.  Na komputerze HoloLens 2 — zaimportuj certyfikat fiddler.
    ...
    6. Upewnij się, że certyfikat jest pokazywany na liście certyfikatów. Jeśli nie, powtórz te kroki.
...
Wireshark to analizator protokołu sieciowego, który służy do sprawdzania ruchu TCP/UDP do i z urządzeń HoloLens 2. Ułatwia to zidentyfikowanie ruchu, który przekracza sieć, do sieci HoloLens 2 — jaka jest jego częstotliwość, jak duże jest opóźnienie między określonymi przeskokami itd.

#### <a name="prerequisites"></a>Wymagania wstępne:
- Komputer musi mieć dostęp do Internetu i obsługiwać udostępnianie Internetu za pośrednictwem sieci Wi-Fi.
...
1. Na komputerze HoloLens 2 — zmień sieć Wi-Fi na mobilny hotspot komputera. HoloLens 2 ruch IP będzie pokazywany w programie Wireshark.
...
- Urządzenie nie jest wyświetlane jako "Urządzenie przyłączone do usługi Azure AD" [w](https://portal.azure.com/) Azure Portal -> Azure Active Directory -> Urządzenia.

### <a name="cause"></a>Przyczyna
Urządzenie, na które ma to wpływ, zostało usunięte z dzierżawy usługi Azure AD. Na przykład może się tak zdarzyć, ponieważ:

- Administrator lub użytkownik usunął urządzenie w Azure Portal lub przy użyciu programu PowerShell.
- Urządzenie zostało usunięte z dzierżawy usługi Azure AD z powodu braku aktywności. W przypadku wydajnie zarządzanego środowiska zwykle zalecamy administratorom IT usuwanie nieaktywnych, nieaktywnych urządzeń [ze swojej dzierżawy usługi Azure AD.](/azure/active-directory/devices/manage-stale-devices)

Jeśli urządzenie, na które ma to wpływ, spróbuje ponownie skontaktować się z dzierżawą usługi Azure AD po jej usunięciu, uwierzytelnianie w usłudze Azure AD nie powiedzie się. Ten efekt jest często niewidoczny dla użytkownika urządzenia, ponieważ w pamięci podręcznej logowanie za pomocą numeru PIN będzie nadal umożliwiać użytkownikowi logowanie.

### <a name="mitigation"></a>Ograniczanie ryzyka
Obecnie nie ma możliwości dodania usuniętego urządzenia HoloLens z powrotem do usługi Azure AD. Objęte urządzenia muszą być czyste reflashed, zgodnie z instrukcjami [dotyczącymi reflashing ich urządzenia.](hololens-recovery.md#clean-reflash-the-device)

[Powrót do listy](#list)

## <a name="autopilot-troubleshooting"></a>Rozwiązywanie problemów z rozwiązaniem Autopilot

Poniższe artykuły mogą być przydatnym zasobem, który pozwala uzyskać więcej informacji i rozwiązywać problemy z rozwiązaniem Autopilot. Artykuły są jednak oparte na programie Windows 10 Desktop i nie wszystkie informacje mogą dotyczyć HoloLens:
...
### <a name="can-i-use-system-center-configuration-manager-sccm-to-manage-hololens-devices"></a>Czy można używać System Center Configuration Manager (SCCM) do zarządzania HoloLens urządzeniami?

Nie. Zarządzanie urządzeniami przenośnymi jest HoloLens MDM.
...
Nie. Aby zarządzać kontami użytkowników Azure Active Directory urządzeń, należy użyć usługi HoloLens Azure Active Directory (Azure AD).

### <a name="is-hololens-capable-of-automated-data-capture-systems-adcs-auto-enrollment"></a>Czy HoloLens automatyczne rejestrowanie za pomocą zautomatyzowanych systemów przechwytywania danych (ADCS)?
...
Nie. Można jednak omiń ten problem, korzystając z jednej z następujących metod:
...
Rejestrowanie jest ograniczone do śladów, które mogą być przechwytywane w scenariuszach testowania lub rozwiązywania problemów, albo danych telemetrycznych, które urządzenia wysyłają do serwerów firmy Microsoft.

## <a name="questions-about-securing-hololens-devices"></a>Pytania dotyczące zabezpieczania HoloLens urządzeń

Zobacz [nasze HoloLens zabezpieczeń 2.](security-overview.md)
Aby HoloLens pierwszej generacji, zapoznaj się z często [zadawanymi pytaniami.](hololens1-faq-security.yml)
