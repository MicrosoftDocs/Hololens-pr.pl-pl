...
Jeśli poprzednia procedura nie zadziałała, spróbuj ponownie uruchomić urządzenie przy [użyciu](/windows/mixed-reality/using-the-windows-device-portal)Windows Portal urządzeń . W prawym górnym rogu znajdź opcję ponownego uruchomienia lub zamknięcia urządzenia.
...
> [!WARNING]
> Jest to potencjalnie szkodliwy sposób i powinna być używana tylko wtedy, gdy wcześniej cytowane metody nie zadziałały.
...
1. Połączenie urządzenie HoloLens do komputera przy użyciu kabla mikro USB, który jest do niego podłączony. (Nawet jeśli używasz innych kabli do łączenia urządzenia HoloLens, ten z nich działa najlepiej).
...
1. Połączenie HoloLens (1. generacja) do komputera przy użyciu kabla Micro USB, który został podłączony do urządzenia HoloLens.
1. Otwórz narzędzie Windows Device Recovery Tool i postępuj zgodnie z instrukcjami.

Jeśli HoloLens (1. generacja) nie zostanie wykryta automatycznie, wybierz pozycję **Nie wykryto mojego urządzenia.** Następnie postępuj zgodnie z instrukcjami, aby przełożyć urządzenie w tryb odzyskiwania.
...
2. Przytrzymaj przycisk **regulacji głośności,** a następnie naciśnij krótko **przycisk** zasilania. Urządzenie powinno uruchamiać się i wyświetlać tylko środkową diodę LED.
