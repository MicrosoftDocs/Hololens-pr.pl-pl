...
description: Dowiedz się, jak współtwo HoloLens do dokumentów na platformie docs.microsoft.com przy użyciu GitHub języka Markdown.
author: hferrone
ms.author: mattwoj
ms.date: 01/04/2021
ms.topic: article
...
ms.locfileid: "123188988"
---
# <a name="contributing-to-the-hololens-documentation"></a>Współtwoowanie dokumentacji HoloLens technicznej

Witamy w [HoloLens dokumentacji!](https://github.com/MicrosoftDocs/Hololens) Wszystkie artykuły, które tworzysz lub edytujesz w tym repocie, **będą widoczne publicznie.** 

HoloLens są wyświetlane na platformie docs.microsoft.com, która używa GitHub markdown z funkcjami parserów Markdig. Zawartość edytowana w tym repo zostanie sformatowana na stronach ze stylizowaną zawartością, które są wyświetlane na poziomie /hololens.
...
| Rzeczywistość mieszana | [MicrosoftDocs/rzeczywistość mieszana](/windows/mixed-reality) |
| Przewodnik po entuzjaściach VR | [MicrosoftDocs/mixed-reality/nagie podręcznik](https://github.com/MicrosoftDocs/mixed-reality/tree/docs/enthusiast-guide) |
...
- Utwórz silne [hasło dla konta GitHub .](https://github.com/settings/admin)
...
- Zaktualizuj ustawienia [profilu publicznego.](https://github.com/settings/profile)
   - Ustaw swoje imię i nazwisko, a następnie rozważ ustawienie dla opcji *Publiczny adres e-mail* wartości Nie *pokazuj mojego adresu e-mail*.
   - Zalecamy przekazanie obrazu profilu, ponieważ miniatura jest wyświetlana na stronach dokumentów, które współtworzsz.
- Jeśli planujesz używać wiersza polecenia, rozważ skonfigurowanie usługi [Git Menedżer poświadczeń dla Windows](https://github.com/Microsoft/Git-Credential-Manager-for-Windows/releases/latest). Dzięki temu nie będzie trzeba wprowadzać hasła za każdym razem, gdy wprowadzasz swój wkład.
...
2. Wybierz przycisk edycji (ikona ołówka) w prawym górnym rogu.
   ...
   > [!NOTE]
   > Ten artykuł zawiera odwołania do _głównego_ terminu , który nie jest już używany przez firmę Microsoft. Po usunięciu terminu z oprogramowania usuniemy go z tego artykułu.
...
4. Zaktualizuj metadane w górnej części każdego artykułu:
   ...
   * **description:** Napisz krótki opis zawartości artykułu, co przyspiesza optymalizację pod kątem wyszukiwarek i odnajdywanie.
   * **author:** Jeśli jesteś głównym właścicielem strony, dodaj tutaj swój alias GitHub .
   ...
   * **ms.date:** zaktualizuj datę w przypadku dodawania głównej zawartości do strony, ale nie w przypadku poprawek, takich jak wyjaśnienie, formatowanie, gramatyka lub pisownia.
   * **keywords:** Słowa kluczowe wspomagają optymalizację pod kątem wyszukiwarek( SEO). Dodaj słowa kluczowe rozdzielone przecinkiem i spacją, które są specyficzne dla Twojego artykułu, ale nie mają interpunkcji po ostatnim sdaniu kluczowym na liście. Nie musisz dodawać globalnych słów kluczowych, które mają zastosowanie do wszystkich artykułów, ponieważ są one zarządzane w innym miejscu. 
   
5. Po zakończeniu edycji artykułu przewiń w dół i wybierz pozycję **Zaproponuj zmianę pliku.**

6. Na następnej stronie wybierz pozycję Utwórz żądanie **ściągnięcie,** aby scalić automatycznie utworzoną gałąź z gałęzią domyślną _master_.
...
- To `redirect_url` względny publiczny adres URL ze starego artykułu do nowego artykułu. Upewnij się, że **ten** adres URL nie zawiera adresu lub , ponieważ odwołuje się on do publicznego adresu URL, a `mixed-reality-docs` nie do ścieżki `.md` repozytorium. Dozwolone jest tworzenie linków do sekcji w nowym artykule `#section` przy użyciu . W razie potrzeby możesz również użyć ścieżki bezwzględnej do innej witryny.
...
Poniższy przepływ pracy umożliwia *tworzenie nowych artykułów w* repo dokumentacji za pośrednictwem GitHub w przeglądarce internetowej:

1. Utwórz widelec poza gałęzią domyślną _master_(master) dokumentu MicrosoftDocs/rzeczywistości mieszanej, używając **przycisku Fork (Utwórz** widelec) w prawym górnym rogu.

   ![Rozgałęzienie gałęzi domyślnej o nazwie "master".](images/forkbranch.png)
...
9. Wybierz **pozycję Nowe żądanie ściągnięcie** i scal gałąź master twojego widelecu z gałęzią MicrosoftDocs/wzorcem rzeczywistości mieszanej (upewnij się, że strzałka wskazuje prawidłowe miejsce docelowe).  

   ![Utwórz żądanie ściągnięcie z twojego widelecu do dokumentu MicrosoftDocs/mixed-reality.](images/pr-to-master.png)
...
Rozszerzenie [Docs Markdown dla](/teamblog/docs-extension) Visual Studio Code ułatwia również generowanie tabel, jeśli używasz narzędzia [Visual Studio Code (patrz poniżej)](#using-visual-studio-code) do edytowania dokumentacji.
...
>[!NOTE]
>Wszystkie powyższe wskazówki dotyczące [edytowania](#editing-an-existing-article) i [tworzenia](#creating-a-new-article) artykułów oraz podstawy edytowania znaczników [Markdown](#markdown-basics)mają zastosowanie również w przypadku Visual Studio Code artykułów.
