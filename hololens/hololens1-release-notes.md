...
HoloLens (1. generacja) ma stan Long Term Servicing (LTS). Przyszłe aktualizacje skupią się na problemach i poprawkach zabezpieczeń, zachowując równowagę funkcji z wersją Windows 10 Holographic 1809 wersji 1809 programu HoloLens (1. generacja).
...
| Cecha | Szczegóły |
|---|---|
| **Menu Szybkie akcje** | Gdy jesteś w aplikacji, gest Blooma otwiera teraz menu Szybkie akcje, aby zapewnić szybki dostęp do często używanych funkcji systemu bez konieczności opuszczania aplikacji. <br> Zobacz [Konfigurowanie HoloLens w trybie kiosku,](hololens-kiosk.md) aby uzyskać informacje o menu Szybkie akcje w trybie kiosku.<br><br> |
...
| **Project do urządzenia Miracast z włączoną obsługą usługi** | Project zawartość HoloLens na pobliskim urządzeniu Surface lub w odbiorniku TV/Monitor, jeśli korzystasz z adaptera Microsoft Display.  Na **stronie Start** **Połączenie** pozycję , a następnie wybierz urządzenie, na którym chcesz projektować. **Uwaga:** Możesz wdrożyć aplikację HoloLens, aby używać Miracast projekcji bez włączania trybu dewelopera. |
...
| **HoloLens nakładane**<br>(s wyboru plików, klawiatury, okien dialogowych itp.) | Podczas korzystania z aplikacji immersywnych będą teraz wyświetlane nakładki, takie jak klawiatura, okna dialogowe, s wyboru plików itp. |
...
| **Nowy interfejs użytkownika do rozruchu urządzenia** | Podczas procesu rozruchu dodano wskaźnik ładowania w celu zapewnienia wizualnej opinii o ładowaniu systemu. Uruchom ponownie urządzenie, aby zobaczyć nowy wskaźnik ładowania — jest on między komunikatem "Hello" a Windows rozruchowym. |
...
| **Udostępnianie z Microsoft Edge** | Przycisk Udostępnij jest teraz dostępny Microsoft Edge oknach HoloLens. W Microsoft Edge wybierz pozycję **Udostępnij**. Użyj s HoloLens udostępniania zawartości internetowej. |
...
| Przypisany dostęp za pomocą grup usługi Azure AD | Grupy usługi Azure AD można teraz używać do konfigurowania Windows dostępu w celu skonfigurowania konfiguracji kiosku z jedną lub wieloma aplikacjami. |
...
Windows 10 wersji 1803 jest pierwszą aktualizacją funkcji w wersji Windows Holographic for Business od jej wydania w wersji Windows 10 wersji 1607. Ta aktualizacja wprowadza następujące zmiany:
...
- Możesz skonfigurować [HoloLens jako kiosk z pojedynczą aplikacją lub z wieloma aplikacjami.](hololens-kiosk.md) Tryb kiosku z wieloma aplikacjami umożliwia skonfigurowanie zasad HoloLens tylko tych aplikacji, które określisz, i uniemożliwia użytkownikom wprowadzenie zmian.
