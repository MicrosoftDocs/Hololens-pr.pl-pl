...
Na wysokim poziomie administrator IT zazwyczaj tworzy konfiguracje gotowe do użycia w firmie i rejestruje 2 urządzenia HoloLens portalach MDM. Gdy HoloLens 2 uruchamiają się przy użyciu gotowego do użycia środowisko OOBE (out-of-box experience) i łączą się z Internetem, konfiguracje gotowe do użycia w firmie dla zarejestrowanego urządzenia z systemem HoloLens 2 są automatycznie pobierane i stosowane, aby przygotować urządzenie do pracy bez żadnej interwencji użytkownika.
...
1. W wyodrębnione pliki, znajdź plik CSV, który ma prefiks nazwy pliku "DeviceHash". Skopiuj ten plik na dysk na komputerze, na którym można uzyskać do niego dostęp później.  
...
1. Na stronie **Przeglądanie + tworzenie** przejrzyj ustawienia, a następnie wybierz pozycję **Utwórz,** aby utworzyć profil.  
   ...
   > [!div class="mx-imgBorder"]
   > ![Przejrzyj i utwórz.](./images/hololens-ap-profile-summ.png)
...
1. W Microsoft Endpoint Manager administracyjnym wybierz pozycję **Urządzenia** Windows  >    >  **Windows rejestracji**  >  **urządzeń.**
...
1. Środowisko rozwiązania Autopilot wymaga dostępu do Internetu. Aby zapewnić dostęp do Internetu, użyj jednej z następujących opcji:
    ...
    - Połączenie urządzenie do sieci Wi-Fi OOBE, a następnie pozwól, aby automatycznie wykrywało środowisko rozwiązania Autopilot. Jest to jedyny czas, gdy trzeba będzie korzystać z rozwiązania OOBE, dopóki środowisko rozwiązania Autopilot nie zostanie ukończone samodzielnie. Domyślnie 2 HoloLens 10 sekund na wykrycie rozwiązania Autopilot po wykryciu Internetu. Jeśli w ciągu 10 sekund nie zostanie wykryty żaden profil rozwiązania Autopilot, OOBE przedstawi eula. Jeśli napotkasz ten scenariusz, uruchom ponownie urządzenie, aby można było spróbować wykryć autopilot. Należy również pamiętać, że funkcja OOBE może przez czas nieokreślony czekać na funkcję Autopilot tylko wtedy, gdy na urządzeniu ustawiono zasady TenantLockdown.
    ...
    - Połączenie urządzenia z adapterami "USB-C do Wi-Fi" na bezprzewodowe połączenie z Internetem i pozwól na automatyczne HoloLens 2 pełne środowisko rozwiązania Autopilot.
        ...
        > [!IMPORTANT]  
       > Urządzenia próbujące używać sieci Wi-Fi oobe dla rozwiązania Autopilot muszą znajdować się na Windows Holographic w wersji [20H2.](hololens-release-notes.md#windows-holographic-version-20h2)
       >
...
HoloLens 2 obsługują usługę TenantLockdown CSP od Windows Holographic w wersji 20H2. Ten program CSP utrzymuje urządzenia w dzierżawie organizacji, blokując je do tej dzierżawy nawet za pośrednictwem resetowania urządzenia lub reflash.

[TenantLockdown](/windows/client-management/mdm/tenantlockdown-csp) Zasady CSP umożliwiają HoloLens 2 z rejestracją MDM tylko przy użyciu rozwiązania Autopilot. Gdy w węźle RequireNetworkInOOBE w węźle RequireNetworkInOOBE dla węzła TenantLockdown zostanie ustawiona wartość true lub false (początkowo ustawiona) na HoloLens 2, ta wartość pozostanie na urządzeniu pomimo reflashingu, aktualizacji systemu operacyjnego itp.
...
#### <a name="what-would-happen-during-oobe-if-autopilot-profile-is-unassigned-on-a-hololens-after-tenantlockdown-was-set-to-true"></a>Co się stanie podczas OOBE, jeśli profil rozwiązania Autopilot zostanie nieprzypisane na HoloLens gdy ustawienie TenantLockdown zostanie ustawione na wartość true? 
