...
1. Określ rodzaj konta użytkownika logując się do usługi HoloLens w swoim środowisku — usługa HoloLens obsługuje konta usługi Azure Active Directory (AAD), konta Microsoft (MSA) i konta lokalne. Ponadto są również obsługiwane tymczasowo utworzone konta o nazwie goście/goście (tylko w przypadku urządzeń przyłączanych do usługi AAD). Aby dowiedzieć się [więcej, zobacz Manage user identity and sign-in for HoloLens (Zarządzanie](hololens-identity.md)tożsamością użytkownika i logowaniem na HoloLens).
...
4. Określ, czy tryb kiosku zostanie zastosowany do usługi HoloLens za pośrednictwem pakietów aprowiwizowania środowiska uruchomieniowego lub serwera mobile Zarządzanie urządzeniami (MDM).
...
- Jeśli chcesz kontrolować dostęp do określonych możliwości sprzętowych, na przykład aparatu, Bluetooth itp. dla niektórych aplikacji itp., zobacz Zasady w programie [Policy CSP supported by HoloLens 2 - Windows Client Management](/windows/client-management/mdm/policies-in-policy-csp-supported-by-hololens2)( Zasady w programie Policy CSP obsługiwanym przez program HoloLens 2 — Windows Client Management). Możesz zapoznać się z [naszymi wspólnymi ograniczeniami urządzeń,](hololens-common-device-restrictions.md) aby zapoznać się z pomysłami.
- Tryb kiosku nie blokuje uruchamiania innych aplikacji przez aplikację (skonfigurowaną w ramach obsługi kiosku). Jeśli chcesz całkowicie zablokować uruchamianie niektórych aplikacji/procesów w usłudze HoloLens, zobacz Używanie usługi Windows Defender Application Control na urządzeniach z systemem [HoloLens 2](/mem/intune/configuration/custom-profile-hololens) na platformie Microsoft Intune — Azure
...
- Począwszy od Windows Holographic w wersji [21H1,](hololens-release-notes.md#windows-holographic-version-21h1)tryb kiosku wyszukuje dostęp przypisany globalnie, zanim zostanie wyświetlane puste menu Start. Środowisko kiosku powróci do globalnej konfiguracji kiosku (jeśli występuje), jeśli wystąpią błędy podczas trybu kiosku grupy usługi AAD.

**Kroki rozwiązywania problemów**

- Sprawdź, czy wartość AUMID aplikacji jest poprawnie określona i czy nie zawiera wersji. Przykłady [można HoloLens aumidami](hololens-kiosk-reference.md#hololens-application-user-model-ids-aumids) aplikacji skrzynki odbiorczej.
