...
\*Aby uzyskać więcej informacji na temat sposobu włączania wyłączonych funkcji lub sposobu interakcji poleceń głosowych z wyłączonymi funkcjami i Cortana zobacz [HoloLens AUMID for apps](hololens-kiosk-reference.md#hololens-application-user-model-ids-aumids)(Identyfikatory AUMID dla aplikacji).
...
1. Określ rodzaj konta użytkownika logując się do usługi HoloLens w swoim środowisku — usługa HoloLens obsługuje konta usługi Azure Active Directory (AAD), konta Microsoft (MSA) i konta lokalne. Ponadto tymczasowo utworzone konta nazywane gośćmi/gośćmi są również obsługiwane (tylko w przypadku urządzeń przyłączanych do usługi AAD). Aby dowiedzieć się [więcej, zobacz Manage user identity and sign-in for HoloLens](hololens-identity.md)(Zarządzanie tożsamością użytkownika i logowaniem na HoloLens).
...
4. Określ, czy tryb kiosku zostanie zastosowany do usługi HoloLens za pośrednictwem pakietów aprowiwizowania środowiska uruchomieniowego lub serwera Zarządzanie urządzeniami (MDM).
...
| Określony użytkownik, który się inseruje, otrzymuje środowisko kiosku.  | [Skonfiguruj profil dostępu przypisany do jednej lub wielu aplikacji (zgodnie z potrzebami), określając nazwę określonego użytkownika.](hololens-kiosk-reference.md#multiple-app-assigned-access-profile-for-a-local-account-or-aad-user-account) | [Zobacz obsługiwane opcje poniżej.](#steps-in-configuring-kiosk-mode-for-hololens) | W przypadku trybu kiosku z pojedynczą aplikacją tylko konto użytkownika lokalnego lub konto MSA jest obsługiwane HoloLens. <br> W przypadku wielu trybów kiosku aplikacji tylko konto MSA lub konto usługi AAD jest obsługiwane HoloLens. |
...
| Użytkownicy w różnych grupach aAD mają tryb kiosku, który jest tylko dla ich grupy. | [Skonfiguruj konfigurację przypisanego dostępu dla każdej żądanej grupy usługi AAD.](hololens-kiosk-reference.md#multiple-app-assigned-access-profile-for-two-aad-groups-or-more) | • [Microsoft Intune szablon niestandardowy](hololens-kiosk.md?tabs=intunecustom#steps-in-configuring-kiosk-mode-for-hololens) <br> • [Aprowizowanie środowiska uruchomieniowego — wiele aplikacji](hololens-kiosk.md?tabs=ppkgmak#steps-in-configuring-kiosk-mode-for-hololens) | • Gdy użytkownik jest połączony z Internetem i HoloLens, jeśli zostanie on członkiem grupy usługi AAD, dla której istnieje konfiguracja kiosku, użytkownik otrzyma dostęp do kiosku dla tej grupy usługi AAD. <br> • Jeśli podczas logowania użytkownika nie jest dostępny Internet, użytkownik będzie HoloLens [zachowanie trybu awarii.](#issue---no-apps-are-shown-in-start-menu-in-kiosk-mode) <br> • Jeśli dostępność Internetu nie jest gwarantowana, gdy należy użyć kiosku opartego na grupach usługi AAD i użytkownika, rozważ użycie zasad [AADGroupMembershipCacheValidityInDayspolicy.](hololens-release-notes.md#cache-azure-ad-group-membership-for-offline-kiosk) |
...
### <a name="how-can-visitor-accounts-automatically-logon-to-kiosk-experience"></a>Jak konta gości mogą automatycznie logować się do kiosku?
