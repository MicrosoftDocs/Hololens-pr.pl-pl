...
|-------------------------|----------------------------|--------------|------------------|
| [Przenoszenie trybu platformy](#moving-platform-mode) | Wprowadza tryb przenoszenia platformy w wersji beta, który po skonfigurowaniu umożliwia korzystanie z wersji HoloLens 2 w dużych jednostkach kamiejowych, w których występuje ruch o niskiej dynamice. | Wszystko | 20348.1411 |
...
Teraz dodane w kompilacjach Windows Insider istnieją dwie formy informacji zwrotnych dla diagnostyki w trybie offline. Pierwsze z nich to wyskakujące powiadomienia wyświetlane zarówno podczas uruchamiania, jak i zakończenia zbierania. Będą one wyświetlane, gdy użytkownik jest zalogowany i ma wizualizacje.
...
- Włączono [interfejs API przypisanego dostępu,](/uwp/api/windows.system.userprofile.assignedaccesssettings?view=winrt-20348&preserve-view=true) dzięki czemu aplikacje mogą teraz określić, HoloLens działa w trybie kiosku dla użytkownika zalogowanego do HoloLens.
...
- Rozwiązaliśmy problem, który oznaczał, że po pierwszym zalogowaniu użytkownika działanie OOBE było przerywane w scenariuszach, w których były używane konfiguracje kiosku oparte na grupach usługi AAD.
...
Windows niejawny program testów jest teraz przenoszący się do kanałów. Szybki **pierścień** stanie się kanałem **dewelopera,** pierścień Wolny stanie  się **kanał beta,** a pierścień wersji zapoznawczej kanałem **wersji zapoznawczej**.  Oto jak wygląda to mapowanie:
